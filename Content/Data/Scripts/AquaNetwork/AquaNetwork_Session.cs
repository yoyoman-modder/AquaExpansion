...
namespace AquaNetwork
{
    ...
    public class AquaNetwork_Session : MySessionComponentBase
    {
        ...
        public override void LoadData()
        {
            ...
            AquaNetLog = new AquaPacketNetLog();
            ...
            AquaPacketNetLog.OnReceive += AquaNetLog_OnReceive;
        ...
        private void AquaNetLog_OnReceive(AquaPacketNetLog packet, ref PacketInfo packetInfo, ulong senderSteamId)
        {
            string msg = $"[AquaNetLog] Received AQUANETLOG TEST";
        ...
        public void nNetLog(string message)
        {
            if (!MyAPIGateway.Multiplayer.IsServer)
                return;

            if (AquaNet == null || AquaNetLog == null)
                return;

            AquaNetLog.Setup(message);
            MyLog.Default.WriteLineAndConsole(
                "[AquaNet] Sending: " + message);
            AquaNet.SendToServer(AquaNetLog);
        }
        public override void UpdateAfterSimulation()
        {
            ...
            {
                PacketExample.Setup("Control was pressed", MyRandom.Instance.Next());
                MyAPIGateway.Utilities.ShowNotification($"[Example] Sent: text={PacketExample.Text}; number={PacketExample.Number}");
                AquaNet.SendToServer(PacketExample);
            }
            if (MyAPIGateway.Input.IsNewKeyPressed(MyKeys.M))
            {
                PacketExample.Setup("M was pressed", MyRandom.Instance.Next());
                MyAPIGateway.Utilities.ShowNotification($"[Example] Sent: text={PacketExample.Text}; number={PacketExample.Number}");
                AquaNet.SendToServer(PacketExample);
            }*/
        }

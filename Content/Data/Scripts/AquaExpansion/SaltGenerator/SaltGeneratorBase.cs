...
namespace AquaExpansion.SaltGenerator
{
    [MyEntityComponentDescriptor(typeof(MyObjectBuilder_Refinery), false, "SaltExtractor")]
    public class SaltGeneratorBase : MyGameLogicComponent
    {
        ...
        private void UpdateGeneratorOutput()
        {
            ...
            if (outputInv == null)
                return;
            MyFixedPoint currentSalt = 0;
            for (int i = 0; i < outputInv.ItemCount; i++)
            {
                var item = outputInv.GetItemAt(i);
                if (item.HasValue && item.Value.Type.SubtypeId == "SaltRaw")
                {
                    currentSalt += item.Value.Amount;
                }
            }
            // Reset
            if (currentSalt < lastSaltAmount)
            {
                lastSaltAmount = currentSalt;
                return;
            }
            if (lastSaltAmount == 0)
            {
            ...
            var produced = currentSalt - lastSaltAmount;
            if (produced <= 0)
                return;
            ...
            var salt = new MyObjectBuilder_Ingot()
            {
                SubtypeName = "SaltRaw"
            };
            outputInv.AddItems((MyFixedPoint)bonus, salt);
            lastSaltAmount = currentSalt;
        }
